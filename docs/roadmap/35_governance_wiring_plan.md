...
| 35.5 | **Refus de `data_product` hors publication certifiée** (ajoutée le 13 septembre 2026, clôt le premier point du §8 — décision humaine : refuser plutôt que router) : `run_process_and_split` et `run_union_sources_to_table` lèvent `NotImplementedError` dès que le schéma déclare `data_product`, au même endroit et dans le même style que leurs refus MV / streaming, avant toute lecture ou écriture ; le message oriente vers `run_process_to_table` | `core/patterns.py`, `tests/test_patterns.py`, `CHANGELOG.md` | schéma `data_product` → refus sans `process_schema`, lecture de source ni écriture ; schéma sans `data_product` inchangé | trivial |
