...
| Store | `SqliteCertificationStore.get_run/get_latest_promoted/list_history` (35.0) ; `get_latest_promoted`, `get_contract` en lecture (35.2) (`observability/certification_store.py`) |
...
|---|---|---|---|---|
| 35.0 | **Ordre déterministe des événements de run** : (a) dans `publication.py`, l'`occurred_at` de chaque événement d'un run est strictement croissant — `max(maintenant, dernier occurred_at du run + 1 µs)` — ce qui couvre Sqlite **et** Delta sans nouvelle primitive backend ; (b) `SqliteCertificationStore.get_run/get_latest_promoted/list_history` départagent les égalités par ordre d'insertion (`rowid DESC`), sans migration | `observability/publication.py`, `observability/certification_store.py`, `tests/test_certification_store.py`, `tests/test_certified_publication.py` | horloge figée (tous les `now()` identiques) : l'état courant est le dernier écrit et un run se promeut ; événements à égalité d'horodatage insérés directement dans le store : le dernier inséré gagne ; les échecs `Illegal publication transition` de la base disparaissent | standard |
